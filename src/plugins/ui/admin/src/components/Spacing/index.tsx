...
export const SpacingInput = React.forwardRef<HTMLInputElement, any>((props, ref) => {
  ...
  if (!label) {
    // Fallback: Extract the field name from the dot-notation path (e.g. "cardType1.Padding1" -> "Padding1")
  ...
  return (
    <Field.Root name={name} id={name} error={error} required={required}>
      ...
      <TextInput
        ...
        value={value || ''}
        placeholder="Enter top, right, bottom, left"
        aria-label={label}
      />
      <Field.Hint />
      <Field.Error />
